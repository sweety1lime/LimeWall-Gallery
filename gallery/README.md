...
## Подпись каталога (Ed25519)

Опционально: каталог можно **подписывать**, тогда клиент отвергнет подделанный
`catalog.json` даже при компрометации репозитория. Пока ключ не создан —
подпись выключена, и всё работает как раньше (HTTPS + SHA-256).

**Включить (один раз):**
1. `node gallery/keygen.mjs` — создаёт `gallery/pubkey.ed25519` (коммитится,
   вшивается в клиент) и `gallery/private-key.ed25519` (**НЕ коммитить**, хранить
   офлайн — это весь корень доверия).
2. Пересобрать и **выпустить приложение** (публичный ключ вшивается в бинарь).

**При каждом изменении каталога:**
```
node gallery/build-catalog.mjs
node gallery/sign-catalog.mjs      # создаёт catalog.json.sig
```
Коммить `catalog.json` и `catalog.json.sig` вместе. CI проверит подпись
(`verify-signature.mjs`). Потерял приватный ключ → нужен новый keygen и новая
сборка приложения с новым публичным ключом.
